...
public class StaticMethodTransformer<T> implements InvocationHandler {
  ...
  public StaticMethodTransformer(Class<T> target) throws InstantiationException {
    fakeInstance =
        kernbeisser.Security.Proxy.injectMethodHandler(
            Tools.createWithoutConstructor(target),
            new MethodListener() {
              @Override
              public void preMethod(Method original) {
                if (!original.isAnnotationPresent(StaticAccessPoint.class)) {
                  throw new UnsupportedOperationException(
                      "cannot call method " + original + " from static context");
                }
              }
  ...
  public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
    if (method.isAnnotationPresent(StaticAccessPoint.class)) {
      ...
      throw new UnsupportedOperationException(
          "function: "
              + method
              + " does not support calling from static scope, annotate with @StaticAccessPoint");
  }

  public static <T extends StaticInterface> T createStaticInterface(
      Class<T> interfaceClass, Class<? extends T> target) {
    try {
      return (T)
          Proxy.newProxyInstance(
              ClassLoader.getSystemClassLoader(),
              new Class[] {interfaceClass},
              new StaticMethodTransformer(target));
