...
public class PriceCalculator {
    ...
    public static int getShoppingItemPrice(ShoppingItem item,int userSurcharge){
        return getPrice((item.isWeighAble() ? (item.getItemNetPrice()/1000f)*item.getAmount()*item.getItemAmount() : item.getItemNetPrice()*item.getItemAmount()),item.isVatLow() ? 7 : 19,item.getDiscount() == CONTAINER_DISCOUNT ? 1 : item.getDiscount(),item.getDiscount() == CONTAINER_DISCOUNT ? item.getSurcharge()/2f:item.getSurcharge(),userSurcharge);
    }
}
