...
public class EditSupplierModel implements Model<EditSupplierController> {
    ...
    public boolean nameExists(String name) {
        ...
        try {
            em.createQuery("select id from Supplier where name like :n").setParameter("n",name).getSingleResult();
            return true;
    ...
    public boolean shortNameExists(String name) {
        EntityManager em = DBConnection.getEntityManager();
        try {
            em.createQuery("select id from Supplier where shortName like :n").setParameter("n",name).getSingleResult();
            return true;
        }catch (NoResultException e){
            return false;
        }
