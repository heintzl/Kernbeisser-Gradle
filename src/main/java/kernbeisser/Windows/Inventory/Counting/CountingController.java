...
public class CountingController extends Controller<CountingView, CountingModel> {
    ...
    void setInventoryDateMessage(CountingView countingView) {
        ...
        boolean asWarning = inventoryDate.isBefore(ChronoLocalDate.from(LocalDate.now().atStartOfDay()));
    ...
    String createInventoryDateString(boolean asWarning) {
        ...
        if (asWarning) {
            return String.format("Achtung: Das Inventur-Datum '%s' liegt in der Vergangenheit!", dateString);
        }
