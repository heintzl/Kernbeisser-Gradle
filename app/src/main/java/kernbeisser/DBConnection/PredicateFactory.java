...
public interface PredicateFactory <P>{
    ...
    static <P,V> PredicateFactory <P> in(ExpressionFactory<P,V> expressionFactory, Collection<V> objects){
        return ((source, cb) -> expressionFactory.createExpression(source,cb).in(objects.toArray()));
    }


    static <P,V extends Comparable<? super V>> PredicateFactory<P> between(V inBetween, ExpressionFactory<P,V> a, ExpressionFactory<P,V> b){
        return ((source, cb) -> cb.between(cb.literal(inBetween), a.createExpression(source, cb), b.createExpression(source, cb)));
    }

    static <P> PredicateFactory <P> like(ExpressionFactory<P,String> expressionFactory, String pattern){
    ...
    static <P> PredicateFactory<P> and(PredicateFactory<P> ... predicateFactories){
        return ((source, cb) -> cb.and(Arrays.stream(predicateFactories).map(
                predicateFactory -> predicateFactory.createPredicate(source, cb)
        ).toArray(Predicate[]::new)));
    }

    @SafeVarargs
    static <P> PredicateFactory<P> or(PredicateFactory<P> ... predicateFactories){
        return ((source, cb) -> cb.or(Arrays.stream(predicateFactories).map(
                predicateFactory -> predicateFactory.createPredicate(source, cb)
        ).toArray(Predicate[]::new)));
    }
}
